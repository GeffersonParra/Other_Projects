...
class Fase2State(gameStateManager: GameStateManager?) : State(gameStateManager) {
    override var gsm: GameStateManager? = gameStateManager
    val background = afternoonclass(0, 0.toInt(), gsm)
    private val ground = Texture("tiles/afternoon/tile.png")
    ...
    override fun handleInput() {
        ...
        if (Gdx.input.justTouched() && !music){
            val touchX = Gdx.input.x.toFloat()
            val touchY = (Gdx.graphics.height - Gdx.input.y).toFloat()
            if (touchX >= 770f && touchX <= (770f+indicator.width) && touchY >= 385f && touchY <= 385f+indicator.height){
                music = true
        ...
        if (Gdx.input.justTouched() && !fx){
            val touchX = Gdx.input.x.toFloat()
            val touchY = (Gdx.graphics.height - Gdx.input.y).toFloat()
            if (touchX >= 770f && touchX <= (770f+indicator.width) && touchY >= 200f && touchY <= 200f+indicator.height){
                fx = true
    ...
    override fun render(spriteBatch: SpriteBatch?) {
        ...
        if (showShadow && settings) {
            ...
            if (!music) {
                spriteBatch?.draw(indicator, 770f, 385f)
            }
            else if (music) {
                spriteBatch?.draw(indicator, 845f, 385f)
            }
            if (!fx) {
                spriteBatch?.draw(indicator, 770f, 200f)
            }
            else if (fx) {
